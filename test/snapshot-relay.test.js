...
import { createRelay } from '../plugins/snapshot-relay/relay.js';
import { createEncoder } from '../plugins/snapshot-relay/encoder.js';
import { normalise, DEFAULTS, SNAPSHOT_PATH } from '../plugins/snapshot-relay/core.js';
...
test('settings are corrected, not refused', () => {
  ...
  assert.deepEqual(normalise({ quality: 500, maxWidth: 50, maxAgeMs: -4 }), { quality: 95, maxWidth: 128, maxAgeMs: 0 });
...
function fakeSwitcher() {
  ...
  const server = http.createServer((req, res) => {
    seen.push(req.url);
    if (req.url.startsWith('/api/device/snapshots/inputs/1')) {
