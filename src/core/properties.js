 ...
 *
 * You do not address PROGRAM and PREVIEW here, you address a BUFFER — the
 * letter A, B or C on LivePremier, UP or DOWN on Midra — and which one is on
 * air changes on every take. `bankLetter()` is the only sanctioned way to turn
 * one into the other, and it refuses mid-take rather than guessing, because a
 * write to the wrong buffer during a transition lands on the output. See
 * `vendor/surface/preset.js` and `core/dialect.js`.
 *
 * ## Two catalogues, one panel
 *
 * `vendor/surface/catalogue.json` was generated from a LivePremier and
 * `catalogue-mng.json` from a Pulse 4K, by the same tool. The store's dialect
 * says which applies and where a layer lives; this file asks and renders.
 */

import { presetBanks } from './screens.js';
import { NLC, dialectFor, dialectOrDefault } from './dialect.js';
...
const GROUP_LABELS = {
  ...
  /* Midra keeps size in a group of its own; the LivePremier label above
     already says "and size" because there it is one node. */
  size: 'Size',
  opacity: 'Opacity',
 ...
 */
export function layerSections(store = null) {
  const groups = new Map();
  for (const spec of catalogueFor(store).layer) {
...
/**
 * The catalogue for the platform the store speaks — LivePremier's until the
 * store has said otherwise. Both are vendored from awj-surface, generated by
 * the same tool from each device's own bundle; `core/dialect.js` holds them
 * beside the paths they describe.
 */
...
export const valuesFor = (spec, store = null) =>
  ...
  (spec && spec.enum && catalogueFor(store).enums[spec.enum]) || [];
 ...
 */
export function paramPath(target, spec, store) {
  if (!target || !target.id || !target.bank || target.layer == null || !spec) return null;
  /* Omitting the store means LivePremier, so the pure builders and their
     tests read as they always did. Passing one that has not hydrated means
     no platform yet, and a path spelled for a guess is not a path anyone
     should write to. */
  const dialect = store === undefined ? NLC : dialectFor(store);
  if (!dialect) return null;
  return dialect.layerParamPath(target.id, target.bank, target.layer, spec.path);
}

/** Read one property's current value, or undefined if it is not in the mirror. */
export function readValue(store, target, spec) {
  const path = paramPath(target, spec, store);
 ...
 * the same reason.
 */
export function coerce(spec, raw, store = null) {
 ...
 */
export function writeCmd(target, spec, raw, store) {
  const path = paramPath(target, spec, store);
  if (!path) return null;
  const result = coerce(spec, raw, store);
