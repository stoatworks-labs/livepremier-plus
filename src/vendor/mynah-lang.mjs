 ...
 * Commit:    d51392a074c3658ebdfa529b3e16e0fbbef90afd
 * Synced:    2026-09-13
 * SHA-256:   86355eaaa0246493946493519b371d8d1e1e3b97e11c3601778d718dbca39a02
 *
...
var midraSourceValues = () => {
	const out = ["NONE"];
	for (let i = 1; i <= 16; i++) out.push(`INPUT_${i}`);
	out.push("COLOR");
	return out;
};
var BUILTIN_MIDRA_LAYER_PARAMS = [
	{
		id: "source.input",
		path: [
			"source",
...
var BUILTIN_MIDRA_GROUP_PARAMS = [
	{
		id: "control.xTake",
		path: [
			"control",
...
var LIVEPREMIER = {
	...
	paths: {
		...
		masterMemoryLoad,
		masterMemorySave,
		masterSaveProp: (prop) => masterSaveProp(prop),
		layerMemoryLoad,
		layerMemorySave,
		...
		memoryLabel,
		memoryDelete,
		layerSource: (t, buffer, layer) => layerSource(t, buffer, layer),
		layerGeometry: (t, buffer, layer, prop) => layerPosition(t, buffer, layer, prop),
		layerOpacity: (t, buffer, layer) => layerOpacity(t, buffer, layer),
		layerParam: (t, buffer, layer, tail) => layerParamPath(t, buffer, layer, tail)
	},
	masterFilterWrites(filter) {
		const screens = filter?.screens ? filter.screens.map(screenKey) : range$1(DIMS.screen.min, DIMS.screen.max, screenKey);
		const auxes = filter?.auxes ? filter.auxes.map(auxKey) : range$1(DIMS.aux.min, DIMS.aux.max, auxKey);
		const layers = filter?.layers ? [...filter.layers.native ? ["NATIVE"] : [], ...filter.layers.numbers.map(String)] : ["NATIVE", ...range$1(DIMS.layer.min, DIMS.layer.max, String)];
		...
		return [
			...
			},
			{
				prop: "layerFilter",
				value: layers,
				describe: filter?.layers ? `Only layer ${layers.join(", ")}` : "All layers"
			},
			{
				prop: "categoryFilter",
				value: [...categories],
	...
	sourceValue(family, n) {
		switch (family) {
			...
			case "still": return n !== void 0 && n >= 1 && n <= SOURCES.still ? `STILL_${n}` : `!Still ${n} is out of range — stills are 1 to ${SOURCES.still}`;
			case "live": return n !== void 0 && n >= 1 && n <= SOURCES.live ? `LIVE_${n}` : `!Source ${n} is out of range — live inputs are 1 to ${SOURCES.live}`;
		}
	}
};
var targetKeyLp = (t) => t.kind === "screen" ? screenKey(t.n) : auxKey(t.n);
/**
...
var midraCollection = (t) => t.kind === "screen" ? "screen" : "auxiliaryScreen";
var midraDest = (t) => DeviceObject.item(midraCollection(t), t.n);
var midraPreset = DeviceObject.node("preset");
var midraBank = midraPreset.node("bank");
var midraAuxBank = midraPreset.node("auxBank");
var midraMasterBank = midraPreset.node("masterBank");
var midraMtvw = DeviceObject.node("multiviewer");
var midraMtvwControl = Path.fromWs([
	"device",
	"multiviewer",
	"bankList",
	"control"
...
var midraBankFor = (t) => t.kind === "screen" ? midraBank : midraAuxBank;
var midraBankRoot = (bank) => {
	switch (bank) {
		case "screen": return midraBank;
		case "aux": return midraAuxBank;
		case "master": return midraMasterBank;
		case "multiviewer": return midraMtvw;
		case "layer": throw new Error("Midra 4K has no layer memory bank");
	}
};
var midraLayer = (t, buffer, layer) => midraDest(t).item("preset", buffer).item("liveLayer", layerKey(layer));
var MIDRA = {
	id: "midra",
	...
	paths: {
		...
		transitionProp: (t, prop) => DeviceObject.node("transition").item(midraCollection(t), t.n).node("control").prop(prop),
		transitionParam: (t, tail) => appendTail(DeviceObject.node("transition").item(midraCollection(t), t.n), tail),
		screenMemoryLoad: (slot, t, mode) => midraBankFor(t).node("control").node("load").item("slot", slot).item(midraCollection(t), t.n).item("preset", mode).prop("xRequest"),
		screenMemorySave: (slot, t, mode) => midraBankFor(t).node("control").node("save").item(midraCollection(t), t.n).item("preset", mode).item("slot", slot).prop("xRequest"),
		masterMemoryLoad: (slot, mode) => midraMasterBank.node("control").node("load").item("slot", slot).item("preset", mode).prop("xRequest"),
		...
		memoryLabel: (bank, slot) => (bank === "multiviewer" ? midraMtvw.item("bank", slot) : midraBankRoot(bank).item("slot", slot)).node("control").prop("label"),
		memoryDelete: (bank, slot) => (bank === "multiviewer" ? midraMtvw.item("bank", slot) : midraBankRoot(bank).item("slot", slot)).node("control").prop("xDelete"),
		layerSource: (t, buffer, layer) => midraLayer(t, buffer, layer).node("source").prop("input"),
		layerGeometry: (t, buffer, layer, prop) => midraLayer(t, buffer, layer).node(prop.startsWith("size") ? "size" : "position").prop(prop),
		...
		destinationSource: (t, buffer) => midraDest(t).item("preset", buffer).node("background").node("source").prop(t.kind === "aux" ? "content" : "set")
	},
	masterFilterWrites(filter) {
		const key = (n) => String(n);
		const screens = filter?.screens ? filter.screens.map(key) : range$1(MIDRA_DIMS.screen.min, MIDRA_DIMS.screen.max, key);
		const auxes = filter?.auxes ? filter.auxes.map(key) : range$1(MIDRA_DIMS.aux.min, MIDRA_DIMS.aux.max, key);
		const layers = filter?.layers ? filter.layers.numbers.map(key) : range$1(MIDRA_DIMS.layer.min, MIDRA_DIMS.layer.max, key);
		const categories = filter?.categories ?? MIDRA_CATEGORIES;
		const auxCategories = filter?.categories ? filter.categories.filter((c) => MIDRA_AUX_CATEGORIES.includes(c)) : MIDRA_AUX_CATEGORIES;
		return [
			{
				prop: "screenFilter",
				value: screens,
				describe: filter?.screens ? `Only screen ${screens.join(", ")}` : "All screens"
			},
			{
				prop: "auxFilter",
				value: auxes,
				describe: filter?.auxes ? `Only aux ${auxes.join(", ")}` : "All auxes"
			},
			{
				prop: "screenLayerLiveFilter",
				value: layers,
				describe: filter?.layers ? `Only layer ${layers.join(", ")}` : "All layers"
			},
			{
				prop: "screenLayerBackFilter",
				value: true,
				describe: "Background layer"
			},
			{
				prop: "screenLayerTopFilter",
				value: true,
				describe: "Top layer"
			},
			{
				prop: "screenCategoryFilter",
				value: [...categories],
				describe: filter?.categories ? `Only ${categories.join(", ")}` : "All categories"
			},
			{
				prop: "auxCategoryFilter",
				value: auxCategories.length ? [...auxCategories] : [...MIDRA_AUX_CATEGORIES],
				describe: filter?.categories ? `Aux: ${(auxCategories.length ? auxCategories : MIDRA_AUX_CATEGORIES).join(", ")}` : "All aux categories"
			}
		];
	},
	sourceValue(family, n) {
		switch (family) {
			case "none": return "NONE";
			case "colour": return "COLOR";
			case "still": return "!This platform has no still store on a layer — stills are background frames here, set from the Images page";
			case "live": return n !== void 0 && n >= 1 && n <= MIDRA_INPUTS ? `INPUT_${n}` : `!Source ${n} is out of range — inputs are 1 to ${MIDRA_INPUTS}`;
		}
	}
};
/**
...
var MIDRA_AUDIO = {
	...
	paths: {
		...
		audioLayer: (t, buffer) => midraDest(t).item("preset", buffer).node("audio").node("control").prop("source"),
		/** A routing point's mode, direct source and follow target. */
		mode: (point, n) => midraAudioPoint(point, n).prop("mode"),
		direct: (point, n) => midraAudioPoint(point, n).node("directRouting").prop("source"),
		follow: (point, n, what) => midraAudioPoint(point, n).node(what).prop(what === "followLiveLayer" ? "layer" : what === "followScreen" ? "screen" : "widget"),
		/** Mutes: a screen's or aux's audio, an audio output (whole or per channel), an input's channel on a plug. */
		destinationMute: (t) => DeviceObject.node("audio").item(midraCollection(t), t.n).node("control").prop("mute"),
		outputMute: (key, channel) => {
			const out = DeviceObject.node("audio").item("output", key);
			return (channel === void 0 ? out : out.item("channel", channel)).node("control").prop("mute");
		},
		inputChannelMute: (plugKey, channel) => DeviceObject.node("audio").item("input", plugKey).item("channel", channel).node("control").prop("mute")
	},
	/**
	...
	*/
	inputPlugs(n) {
		return {
			1: ["IN1_SDI_EMBEDDED", "IN1_HDMI_EMBEDDED"],
			2: ["IN2_SDI_EMBEDDED", "IN2_HDMI_EMBEDDED"],
			3: ["IN3_SDI_EMBEDDED"],
			4: ["IN4_SDI_EMBEDDED"],
			5: ["IN5_HDMI_EMBEDDED"],
			6: ["IN6_HDMI_EMBEDDED", "IN6_RJ45_EMBEDDED"],
			7: ["IN7_HDMI_EMBEDDED", "IN7_RJ45_EMBEDDED"],
			8: ["IN8_HDMI_EMBEDDED"],
			9: ["IN9_DP_EMBEDDED"],
			10: ["IN10_DP_EMBEDDED"]
		}[n] ?? [`IN${n}_ACTIVE_PLUG_EMBEDDED`];
	}
};
/** The `control` node of a routing point: where its mode, direct source and follow target live. */
function midraAudioPoint(point, n) {
	switch (point) {
...
function midraBufferForMode(mode, transition) {
...
var PLATFORMS = [LIVEPREMIER, MIDRA];
/** The platform for an id, or LivePremier for anything unrecognised. */
var platformFor = (id) => PLATFORMS.find((p) => p.id === id) ?? LIVEPREMIER;
//#endregion
...
var Parser = class {
	...
	AUDIO_MODEL;
	constructor(tokens, inputLength, platform) {
		this.tokens = tokens;
	...
	*/
	parseRoutingEndpoint(what) {
		if (this.eatKeyword("None")) return { kind: "none" };
		const one = (min, max, word) => this.parseRange(min, max, word);
		...
		if (this.eatKeyword("Input")) {
			...
			return {
				kind: "input",
				unit,
				channels: this.parseOptionalChannels()
			};
		}
		...
		if (this.eatKeyword("Dante")) {
			...
			if (this.eatKeyword("Group")) {
				...
				const v = r.values;
				const first = v[0];
				if (!(v.length === 8 && (first - 1) % 8 === 0 && v.every((n, i) => n === first + i))) {
					this.error("Dante is routed in groups of eight here — say Dante 1 Thru 8, 9 Thru 16, 17 Thru 24 or 25 Thru 32, or Dante Group 1 to 4", at);
					return;
				}
				group = (first - 1) / 8 + 1;
			}
	...
	parseOptionalChannels() {
		if (!this.eatKeyword("Channel")) return void 0;
	...
	*/
	parseRoutingAudio() {
		...
		if (this.eatKeyword("Follow")) {
			...
			} else if (this.eatKeyword("Video")) follow = { what: "content" };
...
function compile(cmd, ctx = {}) {
	...
	if (!p.nativeLayer && (cmd.scope.layers?.native || cmd.filter?.layers?.native)) return fail(`${p.name} has no NATIVE layer — its layers are ${p.dims.layer.min} to ${p.dims.layer.max}`);
	...
	switch (cmd.fn) {
		...
		case "Set": {
			...
			const layerless = !layers && paths.destinationSource !== void 0 && targets.every((t) => t.kind === "aux");
			if (!layers && !layerless) return fail("Set needs a Layer — these are layer parameters, not screen ones");
			if (layerless && (cmd.set.size || cmd.set.position || cmd.set.opacity)) return fail(`An auxiliary screen on ${p.name} has a source and nothing else to set — size, position and opacity need a Screen and a Layer`);
			...
			return {
				ok: true,
				ops,
				summary: layerless ? `Set source on ${targets.map(describeTarget).join(", ")} ${describeMode(mode)}` : `Set ${describeAssignment(cmd.set)} on ${targets.map(describeTarget).join(", ")} layer ${layers.join(", ")} ${describeMode(mode)}`
			};
		}
		case "Delete": {
...
*/
function compileMasterStore(p, slot, mode, filter, fail) {
	const ops = [];
	if (filter?.categories) {
		const unknown = filter.categories.filter((c) => !p.categories.includes(c));
		if (unknown.length) return fail(`${p.name} has no ${unknown.join(", ")} category — its masks are ${p.categories.join(", ")}`);
	}
	...
	for (const w of p.masterFilterWrites(filter && {
		...
		layers: filter.layers ? {
			native: filter.layers.native,
			numbers: filter.layers.numbers.values
		} : void 0,
		categories: filter.categories
	})) ops.push({
		path: p.paths.masterSaveProp(w.prop),
		value: w.value,
		describe: w.describe
	});
	ops.push({
...
var bankOfTargets = (targets) => targets.length > 0 && targets.every((t) => t.kind === "aux") ? "aux" : "screen";
function checkSlot(p, bank, slot) {
...
*/
function assignmentOps(p, ops, set, t, buffer, layer, canvas) {
	...
	if (set.size) {
		...
		ops.push({
			path: paths.layerGeometry(t, buffer, layer, "sizeH"),
			value: h,
			describe: `Width ${h}px`
		}, {
			path: paths.layerGeometry(t, buffer, layer, "sizeV"),
			value: v,
	...
	if (set.position) {
		...
		ops.push({
			path: paths.layerGeometry(t, buffer, layer, "posH"),
			value: h,
			describe: `X ${h}px (layer centre)`
		}, {
			path: paths.layerGeometry(t, buffer, layer, "posV"),
			value: v,
...
function compileRoutingAudio(cmd, ctx, fail) {
	...
	if (audio.action === "MUTE" || audio.action === "UNMUTE") {
		...
		const ops = [];
		switch (to.kind) {
			case "none": return fail("None is not something that can be muted");
			case "lineIn":
			case "media":
			case "custom": return fail(`${describeRouting(to)} has no mute of its own here — mute the input's channels, or the point it feeds`);
			case "screen":
			case "aux":
				for (const n of units(to)) {
					...
					ops.push({
						path: A.paths.destinationMute(t),
						value,
						describe: `${verb} ${describeRouting(to, n)} audio`
					});
				}
				break;
			case "input":
				for (const n of units(to)) {
					...
					for (const plug of A.inputPlugs(n)) for (const ch of channels) ops.push({
						path: A.paths.inputChannelMute(plug, ch),
						value,
						describe: `${verb} ${plug} channel ${ch}`
					});
				}
				break;
			default: for (const n of units(to)) {
				const key = A.outputKey(to.kind, n);
				if (key.startsWith("!")) return fail(key.slice(1));
				if (to.channels) for (const ch of to.channels.values) ops.push({
					path: A.paths.outputMute(key, ch),
					value,
					describe: `${verb} ${describeRouting(to, to.kind === "multiviewer" ? void 0 : n)} channel ${ch}`
				});
				else ops.push({
					path: A.paths.outputMute(key),
					value,
					describe: `${verb} ${describeRouting(to, to.kind === "multiviewer" ? void 0 : n)}`
				});
			}
		}
		return {
			ok: true,
			ops,
			summary: ops.length === 1 ? ops[0].describe : `${verb} ${ops.length} audio points`
		};
	}
	if (audio.action === "FOLLOW") {
		...
		const ops = [];
		const label = (n) => describeRouting(to, to.kind === "multiviewer" ? void 0 : n);
		for (const n of units(to)) switch (to.kind) {
			case "screen":
				if (f.what === "layer") {
					ops.push({
						path: A.paths.mode("screen", n),
						value: "FOLLOW_LIVE_LAYER_CONTENT",
						describe: `${label(n)} audio follows live layer content`
					});
					ops.push({
						path: A.paths.follow("screen", n, "followLiveLayer"),
						value: String(f.n),
						describe: `${label(n)} follows layer ${f.n}`
					});
				} else if (f.what === "audioLayer") ops.push({
					path: A.paths.mode("screen", n),
					value: "FOLLOW_AUDIO_LAYER",
					describe: `${label(n)} audio follows its audio layer`
				});
				else return fail(`A screen's audio follows a Layer n or its Audio Layer — not ${f.what === "screen" ? "a screen" : f.what}`);
				break;
			case "aux":
				if (f.what === "content") ops.push({
					path: A.paths.mode("aux", n),
					value: "FOLLOW_CONTENT",
					describe: `${label(n)} audio follows its video content`
				});
				else if (f.what === "audioLayer") ops.push({
					path: A.paths.mode("aux", n),
					value: "FOLLOW_AUDIO_LAYER",
			...
			case "output":
				...
				if (f.n !== void 0) return fail("A video output follows the screen it shows and cannot pick another — say \"Follow Screen\" without a number, or patch a source");
				ops.push({
					path: A.paths.mode("output", n),
					value: "AUTO",
					describe: `${label(n)} audio follows the screen it shows`
				});
				break;
			case "lineOut":
			case "dante":
				if (f.what !== "screen" || f.n === void 0) return fail(`${describeRouting(to)} follows a numbered screen — "Follow Screen 2 On ${describeRouting(to, n)}"`);
				ops.push({
					path: A.paths.mode(to.kind, n),
					value: "FOLLOW_SCREEN",
					describe: `${label(n)} audio follows a screen`
				});
				ops.push({
					path: A.paths.follow(to.kind, n, "followScreen"),
					value: String(f.n),
					describe: `${label(n)} follows screen ${f.n}`
				});
				break;
			case "multiviewer":
				if (f.what !== "widget") return fail("The multiviewer's audio follows a Widget n — \"Follow Widget 3 On Multiviewer\"");
				ops.push({
					path: A.paths.mode("multiviewer"),
					value: "FOLLOW_WIDGET",
					describe: "Multiviewer audio follows a widget"
				});
				ops.push({
					path: A.paths.follow("multiviewer", void 0, "followWidget"),
					value: String(f.n),
					describe: `Multiviewer follows widget ${f.n}`
				});
				break;
			default: return fail(`${describeRouting(to)} is a source, not a point that can follow anything`);
		}
		return {
			ok: true,
			ops,
			summary: ops.length <= 2 ? ops.map((o) => o.describe).join("; ") : `${ops.length} audio writes`
		};
	}
	...
	const fromUnits = from.unit?.values ?? [void 0];
	...
	if (value.startsWith("!")) return fail(`${describeRouting(from, fromUnits[0])} is a destination, not a source — patch from an Input, Dante group, Line Input, Player, Custom or None`);
	if (to.channels) return fail("A point takes its source whole — drop the Channel");
	const srcLabel = describeRouting(from, from.kind === "media" || from.kind === "none" ? void 0 : fromUnits[0]);
	const ops = [];
	if (to.kind === "screen" || to.kind === "aux") {
		...
		for (const n of units(to)) {
			...
			if (!buffer) return fail(`Cannot tell which buffer is ${describeMode(mode)} on ${describeTarget(t)} yet — the device has not reported its take state`);
			ops.push({
				path: A.paths.audioLayer(t, buffer),
				value,
				describe: `${describeTarget(t)} audio layer (${describeMode(mode)}) ← ${srcLabel}`
			});
		}
	} else {
		...
		if (to.kind === "none" || !(to.kind in POINT_NAMES)) return fail(`${describeRouting(to)} is a source, not a destination — patch to a Screen, Aux, Output, Line Output, Dante group or the Multiviewer`);
		for (const n of units(to)) {
			const label = describeRouting(to, to.kind === "multiviewer" ? void 0 : n);
			ops.push({
				path: A.paths.mode(to.kind, n),
				value: "DIRECT_ROUTING",
				describe: `${label} audio: direct routing`
			});
			ops.push({
				path: A.paths.direct(to.kind, n),
				value,
				describe: `${label} ← ${srcLabel}`
			});
		}
	}
	return {
		ok: true,
		ops,
		summary: ops.length === 1 ? ops[0].describe : `${srcLabel} → ${units(to).map((n) => describeRouting(to, to.kind === "multiviewer" ? void 0 : n)).join(", ")}`
	};
}
/**
...
function dispatch$1(msg, ctx) {
	...
	switch (rest.shift()) {
		case "screen": return target(p, { kind: "screen" }, rest, msg, ctx);
		case "aux": return target(p, { kind: "aux" }, rest, msg, ctx);
		case "master": return master(p, rest, msg);
		case "multiviewer": return multiviewer(p, rest, msg);
		default: throw new Error(`unknown address ${msg.address} — after /lp comes screen, aux, master or multiviewer`);
	}
}
function target(p, kind, segs, msg, ctx) {
	...
	switch (segs.shift()) {
		case "take": return trigger(msg, () => [op(p.paths.take(t), true, `Take ${label(t)}`)]);
		case "cut": return trigger(msg, () => [op(groupPath(p, t, "control.xCut", ctx), true, `Cut ${label(t)}`)]);
		case "memory": return memory(p, t, kind.kind, segs, msg);
		case "layer": return layer(p, t, segs, msg);
		case "preset": return preset(p, t, segs, msg, ctx);
		case "group": {
...
function memory(p, t, bank, segs, msg) {
	const slot = index(segs.shift(), "memory", ...slotRange(p, bank));
	const verb = segs.shift();
	switch (verb) {
		case "recall": {
			const mode = presetMode(segs.shift(), msg, "PREVIEW");
			return trigger(msg, () => [op(p.paths.screenMemoryLoad(slot, t, mode), true, `Recall memory ${slot} → ${label(t)} ${mode.toLowerCase()}`)]);
		}
		case "store": {
			const mode = presetMode(segs.shift(), msg, "PROGRAM");
			return trigger(msg, () => [op(p.paths.screenMemorySave(slot, t, mode), true, `Store memory ${slot} ← ${label(t)} ${mode.toLowerCase()}`)]);
		}
		case "label": {
		...
		case "delete": return trigger(msg, () => [op(p.paths.memoryDelete(bank, slot), true, `Delete ${bank} memory ${slot}`)]);
...
function layer(p, t, segs, msg) {
	...
	if (verb === "recall") {
		const mode = presetMode(segs.shift(), msg, "PREVIEW");
		return trigger(msg, () => [op(load(slot, t, mode, l), true, `Recall layer memory ${slot} → ${label(t)} layer ${l} ${mode.toLowerCase()}`)]);
	}
	if (verb === "store") {
		const mode = presetMode(segs.shift(), msg, "PROGRAM");
		return trigger(msg, () => [op(save(slot, t, mode, l), true, `Store layer memory ${slot} ← ${label(t)} layer ${l} ${mode.toLowerCase()}`)]);
	}
...
function preset(p, t, segs, msg, ctx) {
...
function master(p, segs, msg) {
	...
	switch (verb) {
		case "recall": {
			const mode = presetMode(segs.shift(), msg, "PREVIEW");
			return trigger(msg, () => [op(p.paths.masterMemoryLoad(slot, mode), true, `Recall master memory ${slot} → ${mode.toLowerCase()}`)]);
		}
		case "store": return trigger(msg, () => [op(p.paths.masterMemorySave(slot), true, `Store master memory ${slot}`)]);
		case "label": {
			const text = msg.args[0];
			if (typeof text !== "string") throw new Error("a label needs a string argument");
			return [op(p.paths.memoryLabel("master", slot), text, `Label master memory ${slot} "${text}"`)];
		}
		case "delete": return trigger(msg, () => [op(p.paths.memoryDelete("master", slot), true, `Delete master memory ${slot}`)]);
		default: throw new Error(`unknown master action ${verb ?? "(missing)"} — expected recall, store, label or delete`);
	}
}
function multiviewer(p, segs, msg) {
	...
	const verb = segs.shift();
	if (verb === "recall") return trigger(msg, () => [op(p.paths.monitoringMemoryLoad(slot, out), true, `Recall multiviewer memory ${slot} → output ${out}`)]);
	if (verb === "store") return trigger(msg, () => [op(p.paths.monitoringMemorySave(slot, out), true, `Store multiviewer memory ${slot} ← output ${out}`)]);
...
var table = (p, ctx) => ctx.params ?? p.builtinParams;
/** `a, b or c` — the last item joined with "or", as a sentence has it. */
var orList = (items) => items.length <= 1 ? items.join("") : `${items.slice(0, -1).join(", ")} or ${items[items.length - 1]}`;
/** A bank's slot range on this platform, or a refusal naming the platform. */
function slotRange(p, bank) {
...
*/
function presetBuffer(p, t, raw, ctx) {
	const literals = orList(p.buffers.map((b) => b.toLowerCase()));
	if (raw === void 0) throw new Error(`missing preset — expected preview, program, ${literals}`);
	const up = raw.toUpperCase();
	...
	if (!buffer) throw new Error(`cannot tell which buffer is ${mode.toLowerCase()} on ${label(t)} — the device's take state is not known here. Address the buffer directly with ${orList(p.buffers.map((b) => "/" + b.toLowerCase()))}.`);
	return buffer;
}
/**
...
*/
function dictionary(params, platform = LIVEPREMIER) {
	const p = platform;
	...
	for (const kind of ["screen", "aux"]) {
		...
		if (SLOTS.layer) {
			push(g, `/lp/${kind}/{n}/layer/{l}/memory/{slot}/recall`, "none, or 1 to fire", `Recall a layer memory. Layers ${DIMS.layer.min}–${DIMS.layer.max}${p.nativeLayer ? " or NATIVE" : ""}; slots ${SLOTS.layer.min}–${SLOTS.layer.max}.`);
	...
	for (const spec of params.layer) {
		...
		const address = `/lp/screen/{n}/preset/{preview|program|${buffers}}/layer/{l}/${paramAddress(spec.id)}`;
...
//#endregion
export { AUDIO, BUILTIN_GROUP_PARAMS, BUILTIN_LAYER_PARAMS, BUILTIN_MIDRA_GROUP_PARAMS, BUILTIN_MIDRA_LAYER_PARAMS, BUILTIN_MIDRA_PARAMS, BUILTIN_PARAMS, CATEGORIES, DIMS, KEYWORDS, LANGUAGES, LANGUAGE_LABELS, LIVEPREMIER, MIDRA, ROOT as OSC_ROOT, PLATFORMS, Path, SLOTS, VERIFIED_FIRMWARE, coerce, compile, completions, declared, denormalise, keywordTable, midraBufferForMode, dictionary as oscDictionary, paramAddress, paramId, parse, parseLine as parseOsc, platformFor, resolveKeyword, resolve as resolveOsc, run, shortestForm, sniff };
