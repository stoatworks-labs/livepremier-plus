 ...
 *   its position among fields of that shape. Typed text wins over the value the
 *   panel redrew from, so a field whose panel keeps no draft is not reset.
...
/**
 * Follow which dropdown is being picked from.
 *
 ...
 * opened for, Escape, Tab or the caret leaving it closes it. Installed once,
 * at the capture phase, so no panel's own handler can hide it.
 */
export function trackDropdowns(doc = typeof document !== 'undefined' ? document : null) {
  ...
  doc.addEventListener('change', (ev) => { if (ev.target === picking) picking = null; }, true);
  doc.addEventListener('focusout', (ev) => { if (ev.target === picking) picking = null; }, true);
}
...
export function captureFocus(root) {
  ...
  if (textLike(el)) {
    ...
    /* Typed since it was drawn: the value the panel draws from would undo it. */
    token.dirty = el.value !== el.defaultValue;
...
export function restoreFocus(root, token) {
  ...
  if (token.dirty && textLike(match) && match.value !== token.value) match.value = token.value;
